...
{     
    ...
    $settings = DB::table('settings')->where('id', 1)->first();
      ...
      $message = urlencode("Hi, ".$UMessage->username."  You have been merged to payout on ".Config::get('app.name').". check your dashboard to proceed.");
      $sender= urlencode("Infinity");
     ...
     curl_close($ch);  
      ...
      $message = urlencode("Hi, ".$ReciverMessage->username."  You have been marged to receive on ".Config::get('app.name').". check your dashboard to confirm.");
      $sender= urlencode("Infinity");
...
{     $startDate = date('Y-m-d H:i:s');
    $settings = DB::table('settings')->where('id', 1)->first();
      ...
      $UMessage = DB::table('users')->where('id', $row->userid)->first();
      $SenderP = DB::table('userdetails')->where('userid', $row->userid)->first();
      $message = urlencode("Hi, ".$UMessage->username."  You have been marged to payout on ".Config::get('app.name').". check your dashboard to proceed.");
      $sender= urlencode("Inifinity");
      $mobile = $SenderP->phonenumber;
       $url = 'http://www.MultiTexter.com/tools/geturl/Sms.php?username='.Config::get('app.sms-username').'&password='.Config::get('app.sms-passowrd').'&sender='.$sender.'&message='.$message .'&flash=0&recipients='. $mobile;
     $ch = curl_init();
     curl_setopt($ch,CURLOPT_URL, $url);

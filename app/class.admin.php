  ...
  $user = DB::table('userdetails')->where('userid', $row->sender_id)->first();
    echo '<tr>
        ...
        <td>'.$row->payment_status.'</td>';
          if ($row->ProofPic =="") {
           echo '<td><a href="#"><img src="" alt="No POP Uploaded" style="width: 100px;"></a></td>';
          }
          else{ echo '<td><a href="'.App::url().'/images/'.$row->ProofPic.'" target="_BLANK"><img src="'.App::url().'/images/'.$row->ProofPic.'" alt="No POP Uploaded" style="width: 100px;"></a></td>';}
...
function ConfirmActivation($id){
  ...
  $sql = DB::table('activationFee')
      ...
      $BanalnceUp = $BanalnceGet->balance + $ActivationDetails->amount;
           ...
           echo'<div class="alert alert-success" role="alert">
  You have successfully confirm the payment.
</div>';
 ...
 $settings = DB::table('settings')->where('id', 1)->first();
  ...
  $userReceiver = DB::table('userdetails')->where('userid', $row->receiver_id)->first();
    echo '<tr>
        ...
        <td>'.$pack->packname.'</td>
        <td>'.$row->payment_status.'</td>
        <td><a href="'.App::url().'/images/'.$row->ProofPic.'" target="_BLANK"><img src="'.App::url().'/images/'.$row->ProofPic.'" alt="No POP Uploaded" style="width: 100px;"></a></td>
        <td><a data-toggle="collapse" data-parent="#accordion" href="#collapse'.$row->id.'"><button class="btn btn-primary">Disengage</button></a></td>
      </tr>
